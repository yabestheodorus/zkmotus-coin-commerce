...
export async function setupOrderEvents() {
  ...
  paymentContract.on(
    "OrderPaid",
    async (orderId, orderCommitment, amount, serialNumbers) => {
      ...
      try {
        ...
        for (const sn of serialNumbers) {
          ...
          const s = order.serials.find(x =>
            x.serialHash?.toLowerCase() === snStr
          );
        ...
        if (updatedCount === 0) {
          ...
          console.log("Event serials:", serialNumbers.map(s => s.toString()));
          console.log("DB serials:", order.serials.map(s => s.serialHash));
